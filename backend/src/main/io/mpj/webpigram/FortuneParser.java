...
public class FortuneParser {
    ...
    public static String getFortune(String fortuneFile, String datFile) throws IOException {
        try (RandomAccessFile datRaf = new RandomAccessFile(datFile, "r")) {
            ...
            long startPos = Integer.toUnsignedLong(ByteBuffer.wrap(startBuffer).order(ByteOrder.BIG_ENDIAN).getInt());
            long endPos = Integer.toUnsignedLong(ByteBuffer.wrap(endBuffer).order(ByteOrder.BIG_ENDIAN).getInt());
